...
for i in range(len(SB_Sun)):
    ...
    FITSnames = [FITSname0, FITSname2, FITSname3, ]
    MSnames = [MSname0, MSname2, MSname3, ]
    columns = [column_opt[1], column_opt[1], column_opt[1], ]

    for ii in range(0,len(columns)):
        cmdstr = "rm -rf "+ FITSnames[ii] + "*.fits "
        os.system(cmdstr)

    element_run = Parallel(n_jobs=-1)(delayed(imaging_FITS)(setting, columns[k], FITSnames[k], MSnames[k]) for k in range(0,len(FITSnames)))
