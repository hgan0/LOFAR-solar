...
for i in range(len(SB_Sun)):
    ...
    cmdstr = "rm -f " + save_dir_SB + "parmdbm_" + calibrator +"_cycle0.txt"
    os.system(cmdstr)
    #
    # with open( save_dir_SB + "parmdbm_cycle0.txt" , 'w') as f:
    ...
    #     f.write("exit")
    #
    # cmdstr = "parmdbm < " + save_dir_SB + "parmdbm_cycle0.txt"
    # os.system(cmdstr)

    with open( save_dir_SB + "parmdbm_" + calibrator +"_cycle0.txt" , 'w') as f:
        f.write("open tablename='" + save_dir_SB + calibrator +"_"+SB_Calibrator[i]+"_Cycle0_avg.MS/instrument'\n")
        f.write("export Gain* tablename='"+save_dir_SB+ "/" + calibrator +"_cycle0.table'\n")
        f.write("exit")

    cmdstr = "parmdbm < " + save_dir_SB + "parmdbm_" + calibrator +"_cycle0.txt"
    os.system(cmdstr)
